...
/**
 * Add Product Action
 * Handles product creation requests with session/CSRF validation
 * Coordinates with image upload processing and returns JSON responses
...
// Handle image path from coordinated upload (if provided)
$product_image = '';
...
// Validate image path if provided (security check)
if (!empty($product_image)) {
    // Ensure image path is within uploads directory
    if (strpos($product_image, 'uploads/') !== 0) {
        $response['status'] = 'error';
        $response['message'] = 'Invalid image path provided.';
        $response['error_type'] = 'validation_failed';
    ...
    if (strpos($product_image, '..') !== false || strpos($product_image, '//') !== false) {
        $response['status'] = 'error';
        $response['message'] = 'Invalid image path format.';
        $response['error_type'] = 'validation_failed';
        $response['field'] = 'product_image';
        echo json_encode($response);
        exit();
...
try {
    ...
    if (!is_logged_in() || is_session_expired()) {
        ...
        log_session_security_event('session_expired_during_operation', 
            'Session expired while processing product creation', 
            $user_id);
    ...
    // Call controller function to add product with enhanced error handling
    $result = add_product_ctr($product_title, $product_price, $product_description, $product_image, $product_keywords, $category_id, $brand_id, $user_id);
    ...
    if ($result['success']) {
        ...
        error_log("Product created successfully: ID {$result['data']['product_id']}, Title '{$result['data']['product_title']}', Price {$result['data']['product_price']}, User {$user_id}");
        ...
        error_log("Product creation failed for user {$user_id}: {$result['error']} (Type: {$result['error_type']})");
    }
    ...
    error_log("Product creation exception: " . json_encode($exception_context));
    ...
    if (strpos($e->getMessage(), 'session') !== false || 
        ...
        $response['status'] = 'error';
        $response['message'] = 'An unexpected error occurred while creating the product. Please try again.';
