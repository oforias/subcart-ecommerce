...
function validate_session_for_brand_operations()
{
    ...
    if (!is_logged_in()) {
        $validation_result['message'] = 'Authentication required. Please log in to manage brands.';
    ...
    if (!has_admin_privileges()) {
        $validation_result['message'] = 'Access denied. Administrator privileges required to manage brands.';
...
try {
    ...
    if (!is_logged_in() || is_session_expired()) {
        ...
        log_session_security_event('session_expired_during_operation', 
            'Session expired while processing brand creation', 
            $user_id);
    ...
    // Call controller function to add brand with enhanced error handling
    ...
    if ($result['success']) {
        ...
        // Log successful brand creation for audit trail
        error_log("Brand created successfully: ID {$result['data']['brand_id']}, Name '{$result['data']['brand_name']}', Category {$result['data']['category_id']}, User {$user_id}");
        ...
        error_log("Brand creation failed for user {$user_id}: {$result['error']} (Type: {$result['error_type']})");
    }
    ...
    error_log("Brand creation exception: " . json_encode($exception_context));
    ...
    if (strpos($e->getMessage(), 'session') !== false || 
        ...
        $response['status'] = 'error';
        $response['message'] = 'An unexpected error occurred while creating the brand. Please try again.';
