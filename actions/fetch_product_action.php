 ...
 * Retrieves products for the logged-in user organized by categories and brands
...
function validate_session_for_product_operations()
{
    ...
    if (!is_logged_in()) {
        $validation_result['message'] = 'Authentication required. Please log in to access products.';
    ...
    if (!has_admin_privileges()) {
        $validation_result['message'] = 'Access denied. Administrator privileges required to access products.';
...
try {
    ...
    if (!is_logged_in() || is_session_expired()) {
        $response['status'] = 'error';
        $response['message'] = 'Your session has expired. Please log in again to access products.';
        ...
        log_session_security_event('session_expired_during_operation', 
            'Session expired while fetching products', 
            $user_id);
    ...
    // Fetch products for the current user with enhanced error handling
    $result = fetch_products_ctr($user_id, $category_id, $brand_id);
    ...
    if ($result['success']) {
        ...
        // Log successful product fetch for audit trail (only if products exist)
        if ($result['data']['count'] > 0) {
            error_log("Products fetched successfully: {$result['data']['count']} products for user {$user_id}");
        }
        ...
        error_log("Product fetch failed for user {$user_id}: {$result['error']} (Type: {$result['error_type']})");
    }
    ...
    error_log("Product fetch exception: " . json_encode($exception_context));
