 ...
 * Retrieves categories for the logged-in user
...
function validate_session_for_category_operations()
{
    ...
    if (!is_logged_in()) {
        $validation_result['message'] = 'Authentication required. Please log in to access categories.';
    ...
    if (!has_admin_privileges()) {
        $validation_result['message'] = 'Access denied. Administrator privileges required to access categories.';
...
try {
    ...
    if (!is_logged_in() || is_session_expired()) {
        $response['status'] = 'error';
        $response['message'] = 'Your session has expired. Please log in again to access categories.';
        ...
        log_session_security_event('session_expired_during_operation', 
            'Session expired while fetching categories', 
            $user_id);
    ...
    // Fetch categories for the current user with enhanced error handling
    ...
    if ($result['success']) {
        ...
        // Log successful category fetch for audit trail (only if categories exist)
        if ($result['data']['count'] > 0) {
            error_log("Categories fetched successfully: {$result['data']['count']} categories for user {$user_id}");
        }
        ...
        error_log("Category fetch failed for user {$user_id}: {$result['error']} (Type: {$result['error_type']})");
    }
    ...
    error_log("Category fetch exception: " . json_encode($exception_context));
    ...
    if (strpos($e->getMessage(), 'session') !== false || 
        ...
        $response['status'] = 'error';
        $response['message'] = 'An unexpected error occurred while retrieving categories. Please try again.';
