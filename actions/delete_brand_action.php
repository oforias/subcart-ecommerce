...
function validate_session_for_brand_operations()
{
    ...
    if (!is_logged_in()) {
        $validation_result['message'] = 'Authentication required. Please log in to delete brands.';
    ...
    if (!has_admin_privileges()) {
        $validation_result['message'] = 'Access denied. Administrator privileges required to delete brands.';
...
try {
    ...
    if (!is_logged_in() || is_session_expired()) {
        ...
        log_session_security_event('session_expired_during_operation', 
            'Session expired while processing brand deletion', 
            $user_id);
    ...
    // Call controller function to delete brand
    ...
    if ($result['success']) {
        ...
        // Log successful brand deletion for audit trail
        error_log("Brand deleted successfully: ID {$result['data']['brand_id']}, User {$user_id}");
        ...
        switch ($result['error_type']) {
            case 'not_found':
                $response['field'] = 'brand_id';
                $response['suggestion'] = 'The brand may have already been deleted or you may not have access to it.';
                break;
                
            case 'access_denied':
                $response['suggestion'] = 'You do not have permission to delete this brand.';
                break;
            ...
            case 'foreign_key_constraint':
                $response['suggestion'] = 'This brand cannot be deleted because it may be referenced by other records.';
                break;
        }
        
        // Log the error with context for debugging
        error_log("Brand deletion failed for user {$user_id}, brand {$brand_id}: {$result['error']} (Type: {$result['error_type']})");
    }
    ...
    error_log("Brand deletion exception: " . json_encode($exception_context));
    ...
    if (strpos($e->getMessage(), 'session') !== false || 
        ...
        $response['status'] = 'error';
        $response['message'] = 'An unexpected error occurred while deleting the brand. Please try again.';
